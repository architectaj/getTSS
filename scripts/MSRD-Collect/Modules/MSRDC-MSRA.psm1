...
Function msrdGetRemoteHelpData {
    ...
    msrdLogMessage $LogLevel.Normal -LogPrefix $msrdLogPrefix -Message "Remote Help installation and event logs"
    ...
    foreach ($userProfile in Get-ChildItem -Path $msrdUserProfilesDir -Directory -ErrorAction SilentlyContinue) {
        ...
        foreach ($file in $filesToCopy) {
            ...
            $destinationFile = Join-Path $global:msrdSysInfoLogFolder $modifiedFileName
...
Function msrdGetMSRAGroupsMembership {
    ...
    msrdCreateLogFolder $global:msrdSysInfoLogFolder
    msrdGetLocalGroupMembership -logPrefix $msrdLogPrefix -groupName "Offer Remote Assistance Helpers" -outputFile ($global:msrdSysInfoLogFolder + $global:msrdLogFilePrefix + "LocalGroupsMembership.txt")
    msrdGetLocalGroupMembership -logPrefix $msrdLogPrefix -groupName "Distributed COM Users" -outputFile ($global:msrdSysInfoLogFolder + $global:msrdLogFilePrefix + "LocalGroupsMembership.txt")
}
...
Function msrdGetMSRASchedTasks {
    ...
    if ($ScheduledTasks = Get-ScheduledTask -TaskPath '\Microsoft\Windows\RemoteAssistance\' -ErrorAction Ignore) {
        $ScheduledTasks | ForEach-Object -Process {
            $Commands = @(
                "Export-ScheduledTask -TaskName $($_.TaskName) -TaskPath '\Microsoft\Windows\RemoteAssistance' 2>&1 | Out-File -Append '" + $global:msrdSchtaskFolder + $global:msrdLogFilePrefix + "schtasks_" + $_.TaskName + ".xml'"
                "Get-ScheduledTaskInfo -TaskName $($_.TaskName) -TaskPath '\Microsoft\Windows\RemoteAssistance' 2>&1 | Out-File -Append '" + $global:msrdSchtaskFolder + $global:msrdLogFilePrefix + "schtasks_" + $_.TaskName + "_Info.txt'"
            )
...
Function msrdCollectUEX_AVDMSRALog {
    ...
    msrdLogMessage $LogLevel.Info "$(msrdGetLocalizedText "msramsg")"

    if ($varsMSRA[0]) {
        msrdLogMessageAssistMode "Collect Remote Assistance event logs"
        msrdGetMSRAEventLogs
    } #get event logs

    if ($varsMSRA[1]) {
        if (!($global:msrdSource)) {
            msrdLogMessageAssistMode "Collect Remote Assistance registry keys"
            GetMSRARegKeys
        }
    } #get reg keys
    ...
    if ($varsMSRA[2]) {
        if (!($global:msrdSource)) {
            msrdLogMessageAssistMode "Collect Remote Assistance groups membership"
    ...
    if ($varsMSRA[3]) {
        if (!($global:msrdSource)) {
            msrdLogMessageAssistMode "Collect Remote Assistance permissions"
    ...
    if ($varsMSRA[4]) {
        if (!($global:msrdSource)) {
            msrdLogMessageAssistMode "Collect Remote Assistance scheduled tasks"
            msrdGetMSRASchedTasks
        }
    } #get scheduled task

    if ($varsMSRA[5]) {
		msrdLogMessageAssistMode "Collect Remote Help installation and event logs"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDpU9mxZKM9V599
# cz3zaYXDZHPU6fRJdVkMmWPkcOtVi6CCDXYwggX0MIID3KADAgECAhMzAAADrzBA
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIE