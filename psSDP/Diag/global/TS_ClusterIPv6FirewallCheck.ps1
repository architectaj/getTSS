#************************************************
# TS_ClusterIPv6FirewallCheck.ps1
# Version 1.0.1
# Date: 7/11/2012
# Author: v-blchen
# Description:  [Idea ID 2513] [Windows] IPv6 rules for Windows Firewall can cause loss of communications between cluster nodes
# Rule number:  2513
# Rule URL:  http://sharepoint/sites/rules/Rule Submissions/Forms/DispForm.aspx?ID=2513
#************************************************

Import-LocalizedData -BindingVariable ScriptStrings
Display-DefaultActivity -Rule -RuleNumber 2513

$RuleApplicable = $false
$RootCauseDetected = $false
$RootCauseName = "RC_ClusterIPv6FirewallCheck"
$PublicContent = ""
$InternalContent = "https://vkbexternal.partners.extranet.microsoft.com/VKBWebService/ViewContent.aspx?scid=B;en-US;2011797"
$Verbosity = "Warning"
$Visibility = "3"
$SupportTopicsID = "8001"
$Title = $ScriptStrings.ID_ClusterIPv6FirewallCheck_ST

$InformationCollected = new-object PSObject
$Script:FireWallRuleCollection =New-Object PSObject
# ***************************
# Data Gathering
# ***************************
...
# check whether IPv6 was disabled on this domain controller or not
Function IPv6DisabledonDCCheck
{
	$IPv6DisabledComponentsPath = "HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip6\Parameters"
	if(Test-Path $IPv6DisabledComponentsPath)
	{
		$DisabledComponentsValue = (Get-ItemProperty $IPv6DisabledComponentsPath).DisabledComponents
		if(($null -ne $DisabledComponentsValue) -and ($DisabledComponentsValue -eq -1))
		{
			return $true
		}
	}
...
#check whether can apply for current system
Function AppliesToSystem
{
	if(((($OSVersion.Major -eq 6) -and ($OSVersion.Minor -eq 0)) -or (($OSVersion.Major -eq 6) -and ($OSVersion.Minor -eq 1))) -and
		(IsCluster) -and (!(IPv6DisabledonDCCheck)))
		{
			return $true
		}
		else
...
# Check if any of following rules are disabled in Windows Firewall:
...
Function CheckWindowsFirewallRule
{
	$ChekRuleDescriptionCollection = @()
	$ChekRuleDescriptionCollection += @("Core Networking - Neighbor Discovery Advertisement (ICMPv6-Out)")
	$ChekRuleDescriptionCollection += @("Core Networking - Neighbor Discovery Advertisement (ICMPv6-In)")
	$ChekRuleDescriptionCollection += @("Core Networking - Neighbor Discovery Solicitation (ICMPv6-Out)")
	$ChekRuleDescriptionCollection += @("Core Networking - Neighbor Discovery Solicitation (ICMPv6-In)")
	$FirewallRules =(New-object -comObject HNetCfg.FwPolicy2).rules | Where-Object -FilterScript {$ChekRuleDescriptionCollection -contains $_.Name}
	$Ruleflag = $false
	if($null -ne $FirewallRules)
	{
		foreach($rule in $FirewallRules)
		{
			...
			Add-member -InputObject $InformationCollected -Membertype Noteproperty -Name ($rule.Name + " Enabled" ) -Value $rule.Enabled
		}
	}
	return $Ruleflag
}
...
if (AppliesToSystem)
{
	...
	if ($RuleApplicable)
	{
		$RootCauseDetected = CheckWindowsFirewallRule
	}
}	
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCi8ZrXOfkpK+A6
# J4bjkQEG2O/kWCfut0tnlJeHLevBVaCCDXYwggX0MIID3KADAgECAhMzAAACy7d1
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIISVGh7nP3u3XzCD0E+qX6uG
# 9F1b39m+TorC3AFbJuGpMEQGCisGAQQBgjcCAQwxNjA0oBSAEgBNAGkAYwByAG8A
# cwBvAGYAdKEcgBpodHRwczovL3d3dy5taWNyb3NvZnQuY29tIDANBgkqhkiG9w0B
# AQEFAASCAQ