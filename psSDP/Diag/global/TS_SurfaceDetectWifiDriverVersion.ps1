#************************************************
# TS_SurfaceDetectWifiDriverVersion.ps1
# Description: SurfaceDetectWifiDriverVersion
#
# Rule GUID: 151B587B-EF4A-46FC-B263-E975C93962CB
#
# Files:
# TS_SurfaceDetectWifiDriverVersion.ps1
# RC_SurfaceDetectWifiDriverVersion.xml
# Include.xml
#
# Output Files:
# none
...
Write-DiagProgress -Activity $RegKeyCheck.ID_SurfaceDetectWifiDriverVersion -Status $RegKeyCheck.ID_SurfaceDetectWifiDriverVersionDesc


$RootCauseDetected = $false
$HasIssue = $false
$RootCauseName = "RC_SurfaceDetectWifiDriverVersion"
#$PublicContent Title: "Surface update history"
...
if ((isOSVersionAffected) -and (isSurface))
{
	...
	$marvelDriverMPVersionLatest = "MP135"
    $marvelDriverMPVersionLatestInt = 135
	...
	if (test-path $marvelDriver)
	{
		...
		{
			# Retrieves the integer from MP<integer>
			...
			# If the version starts with MP, then continue.
			$marvelDriverProductVersionStartsWithMP = ($marvelDriverProductVersion).StartsWith("MP")
			if ($marvelDriverProductVersionStartsWithMP -eq $true) 
			{
				[int]$marvelDriverProductVersionInt = $marvelDriverProductVersion.Substring(2,$marvelDriverProductVersion.length-2)
				if ($marvelDriverProductVersionInt -lt $marvelDriverMPVersionLatestInt)
				{
					$RootCauseDetected = $true
					add-member -inputobject $InformationCollected -membertype noteproperty -name "Installed   : Marvel Driver Version" -value $marvelDriverVersion
					add-member -inputobject $InformationCollected -membertype noteproperty -name "Installed   : Marvel Driver Product Version" -value $marvelDriverProductVersion
					add-member -inputobject $InformationCollected -membertype noteproperty -name "Recommended : Marvel Driver Version (or later versions)" -value $marvelDriverVersion
					add-member -inputobject $InformationCollected -membertype noteproperty -name "Recommended : Marvel Driver Product Version (or later versions)" -value $marvelDriverMPVersionLatest
				}
			}
		}
	}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBnar1JZU93Wahm
# aHJHcVtf5d61knAOdxHkrESv6ewWsaCCDYEwggX/MIID56ADAgECAhMzAAAB32vw
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQg1f4SAsc3
# vvf8bYHNV/noxkRzDJmlsmDF4FGvSCohsIcwOAYKKwYBBAGCNwIBDDEqMCigCIAG
# AFQAUwBToRyAGmh0dHBzOi8vd3d3Lm1pY3Jvc29mdC5jb20gMA0GCSqGSIb3DQEB
# AQUABIIBABpXdNPk2f+inLkBjmwM4sqSlyxCpNr8H6TgFmk3CjYC6QpqJy0wssZL
# m217YfUBJorOFSXUTO+Li6UCp6u03rg3hYQ+4m5GUzlY4YULRZs8ZCP7a0bfvvkC
# y0PuUjTj9OJy7r2SvSUqUkATz49wCJ9OI/+0DbGpkH3s2f6SuqduemeF+ICn8cGV
# kIjIgglcZxJ2TmgAmMJm8X3OWXj3qQSy+Y4YejCm0HXvaarXMhj5gMnQvmyf0epQ
# wSgj/BmZHb2G3xgZx+loIYzAiKaStGaUdtVx1N1tGCXMeczbct42G1Q+/n1HYVSw
# IJXib+Xxsngg08xnxYUcTkUfXCUtKEyhghLxMIIS7QYKKwYBBAGCNwMDATGCEt0w
...
# ZQMEAgEFAAQg0QthMUsfk6UqIKQ1JlJCIq4b010cnSbrwgnnljJCM1QCBmGB81FC
# bBgTMjAyMTExMTExNjUzMzMuNzY4WjAEgAIB9KCB1KSB0TCBzjELMAkGA1UEBhMC
...
# GgYJKoZIhvcNAQkDMQ0GCyqGSIb3DQEJEAEEMC8GCSqGSIb3DQEJBDEiBCAN1v48
# q3mXAEjuXbppUirVa3wWujVKi44xqSriwC9yAzCB+gYLKoZIhvcNAQkQAi8xgeow
...
# hvcNAQELBQAEggEAfcEgi8eycR0wlW/Pgt5zNBltp29/ABw40IAG0cqhMMnyBt5F
# aHyHfMGO2h76jX+UkfRHFeBUn4HlAb1CDk5JDKFyzvco9NQ3PWsse4/8FpS9vR7m
# lFDLaKl6vtbG8BTuvbvLyE91R/g+9CehRYrNp/VKtDUdtFI6GWFDP4Bg7vsFUsYN
# nQFBh2bXydz8e3hBwkJ6uYBGHzf4JakT7ozoI6yq82RucL5gl0TYr3EZyLj0RFR6
# mqIZq2HDSshRZCSnmTgaKP7D5tO1kPp52+TSfrZVpCae8e4ujIz648i0lyJ5kkgY
# RbtX19hJl7sSsUqmj40LKA8YBHs/ygIpmmSnRA==
