#************************************************
# TS_BitlockerDenyWriteFixedPolicy.ps1
# Version 1.0.1
# Date: 6/14/2012
# Author: v-kaw
# Description:  [Idea ID 2004] [Windows] Bitlocker Drive Preparation fails with Error 'The new active drive cannot be formatted.'
# Rule number:  2004
# Rule URL:  http://sharepoint/sites/rules/Rule Submissions/Forms/DispForm.aspx?ID=2004
#************************************************

Import-LocalizedData -BindingVariable ScriptStrings
Display-DefaultActivity -Rule -RuleNumber 2004

$RuleApplicable = $false
$RootCauseDetected = $false
$RootCauseName = "RC_BitlockerDenyWriteFixedPolicy"
$PublicContent = " http://blogs.technet.com/b/askcore/archive/2010/04/02/how-to-disable-bitlocker-drive-encryption-fixed-data-drive-read-only-policy-using-gpo.aspx"
$InternalContent = ""
$Verbosity = "Error"
$Visibility = "4"
$SupportTopicsID = "8078"
$Title = $ScriptStrings.ID_BitlockerDenyWriteFixedPolicy_ST
...
function CheckDenyWriteAccessPolicy()   #Check if 'Deny write access to fixed drives not protected by BitLocker' policy is currently enabled in the machine 
{
    $Key = "HKLM:\Software\Policies\Microsoft\Fve" 
    if(Test-Path -Path $Key)
	{
	    if(((Get-ItemProperty -Path $Key).FDVEncryptionType) -eq 1)
		{
		    return $true
		}
	}
...
{
	$EventLogName = "Microsoft-Windows-Bitlocker-DrivePreparationTool/Operational"
	$strQuery = "*[System[(EventID=4096 or EventID=4100) and TimeCreated[timediff(@SystemTime) <= 604800000]]]"
	$Result = QueryEventLog $EventLogName $strQuery       #check for Event ID 4096 and 4100 under Microsoft-Windows-Bitlocker-DrivePreparationTool/Operational event log over the past 7 days
	if (($Error.Count -eq 0) -and ($null -ne $Result))
    {
        $EventLogName = "Application"
	    $strQuery = "*[System[Provider[@Name='Microsoft-Windows-Defrag'] and (EventID=258 or EventID=259) and TimeCreated[timediff(@SystemTime) <= 604800000]]]"
	    $Result = QueryEventLog $EventLogName $strQuery	 	  #check for Event ID 258 and 259 from source 'Defrag' under Application event log over the past 7 days
		if (($Error.Count -eq 0) -and ($null -ne $Result))
        {
            return $true      
        }
    }
	return $false    #GreenLight: If 'Deny write access to fixed drives not protected by BitLocker' policy is currently enabled but there are no event logs entries.
}
...
if (AppliesToSystem)
{
	...
	if ((CheckDenyWriteAccessPolicy) -and (CheckEventLogExists -eq $true))  #Check if the policy is currently enabled and fittable event logs exist.
	{
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDwKurZw0Ml2Y9D
# 4NacekucOjTLzVtx8vs2dRqK3DoWb6CCDXYwggX0MIID3KADAgECAhMzAAACy7d1
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIJBp5bL9cdFO3QHuisXno0ZE
# E92hKk4Xh+6SeMGq/7+qMEQGCisGAQQBgjcCAQwxNjA0oBSAEgBNAGkAYwByAG8A
# cwBvAGYAdK