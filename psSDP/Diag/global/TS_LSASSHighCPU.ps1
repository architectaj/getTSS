...
Write-DiagProgress -Activity $RegKeyCheck.ID_LSASSHighCPU -Status $RegKeyCheck.ID_LSASSHighCPUDesc

$RootCauseDetected = $false
$RootCauseName = "RC_LSASSHighCPU"
...
# Description
# -----------
#    This script checks if LSASS Process is using high CPU (> 80%) and fires an alert
#
# Related KB
# -----------
...
if (($IsMachineDC) -and ([System.Environment]::OSVersion.Version.Build -ge 7600))
{
	#Obtain CPU utilization for LSASS process for a few seconds and calculate the average:
	$LSASSCPUSamples = get-counter -Counter "\Process(lsass)\% Processor Time" -SampleInterval 2  -MaxSamples 4  | Select-Object @{Name = "CPUUsage"; Expression = {$_.CounterSamples[0].CookedValue}}
	$Processor = Get-CimInstance -Class Win32_Processor
	$NumberOfLogicalProcessors = ( $Processor| Select-Object -ExpandProperty NumberOfLogicalProcessors | Measure-Object -Sum).Sum
	$LSASSCPUAverage = (($LSASSCPUSamples | measure-object -Property CPUUsage -Average).Average / $NumberOfLogicalProcessors)
	
	#Check if Average CPU usage for LSASS is larger than 80% of each processors
	if ($LSASSCPUAverage -gt 80)
	{		
		$KBArticleNumber = 321044 
		
		$LSASSCPUAverage10 = ("{0,-10:p}" -f ($LSASSCPUAverage/100))
		add-member -inputobject $Return_Object -membertype noteproperty -name "LSASS %CPU Usage Average" -value $LSASSCPUAverage10
		...
		$CurrentValuePerf = new-object PSObject
		add-member -inputobject $CurrentValuePerf -membertype noteproperty -name "Processor Name" -value $Processor.Name
		add-member -inputobject $CurrentValuePerf -membertype noteproperty -name "NumberOfLogicalProcessors" -value $NumberOfLogicalProcessors		
		add-member -inputobject $CurrentValuePerf -membertype noteproperty -name "LSASSCPUAverage" -value $LSASSCPUAverage
		WriteTo-StdOut -ObjectToAdd $CurrentValuePerf 
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD1LXkNnHr2YZxn
# c6G4RRgFwtM+n6XZ/DOQo68PnzRKC6CCDYEwggX/MIID56ADAgECAhMzAAAB32vw
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgGmlQ+feb
# axpTAnOIo0d4QRonqo2EoQNhA5BIZtR9/vAwOAYKKwYBBAGCNwIBDDEqMCigCIAG
# AFQAUwBToRyAGmh0dHBzOi8vd3d3Lm1pY3Jvc29mdC5jb20gMA0GCSqGSIb3DQEB
# AQUABIIBAJmOyf8GgrfOk76EVJbntuC06e4wg48SmXBT3QFz55ZUvJibKovDTbwz
# saNu2z7LMGZhbHcq+9Xn+McKgYQAhFjK0b7SYnm5yKiXZjKko0G8zksvD25cW14M
# U3V6LFN1STeJAEPA07njQ5CYrXLFF8o13MAXwrTf/+kUaD4JJ0PflFBH6WgdSeWH
# hhUouTAYTuqPUYPHMo7A53+Td0ObhCUZPLi30nxFM66H+LoysmjCKpIaHhAaIUZ0
# unopvB/5D9uQhcsxmtN32rWNy1QqY3g8xNWvwZs55Dvw3VB+fzEaMAPY/fxKCyJm
# aYuRtiIKTk5Vm3DzpMrQGU+I6V5uYtGhghLxMIIS7QYKKwYBBAGCNwMDATGCEt0w
...
# ZQMEAgEFAAQg4gnqkVM/RYfk08S+XMjcdbL4Ngs+YC0F5738emrdcxMCBmGB8A0Q
# GBgTMjAyMTExMTExNjUzMzMuNTMyWjAEgAIB9KCB1KSB0TCBzjELMAkGA1UEBhMC
...
# GgYJKoZIhvcNAQkDMQ0GCyqGSIb3DQEJEAEEMC8GCSqGSIb3DQEJBDEiBCCsAll5
# rr/GP+xFc6PIM5t5RS6X2V4zdZkJ7d7DWQCeezCB+gYLKoZIhvcNAQkQAi8xgeow
...
# hvcNAQELBQAEggEAZt98dgV53fqaU9VqQD2gavHNWxqyGN0ZTYMqtc88nl8RdQDA
# xYaFuFxYgZG55CX9OzDzrAmhUxahL15eKSrx805LO+S60MCP4vsT/hhlylT5JQWK
# 5ivvmzoVdU9cFDNlqtqAGy3QBFVOZW0t31p72JfkxomeGQ/zSwa/QGWzY5foV5Cg
# /Baq+xQD5tsjiUPq2ryJboaqSxIuNeoh3yIPZScVRhSg+j9s6dzMvxmIazxLycLw
# flAjjoOXC89hxBGxiBotmWRmRG5BL68euEbmNRfQVibCdcEEAzlC0WvU9u+ZQoMr
# ASx1M84Cmp57bnVL/CAl/lmhrGJgGUs9ZFNnbQ==
