...
$RootCauseName = "RC_HPWMICheck"
$InformationCollected = new-object PSObject
$damagedAutorecoverMofsRegistryKey = $false
$HPWbemUpdateRequired = $false

#detect problem HP version
$Binary = join-path $env:ProgramFiles "HPWBEM\HPWmiVer.dll"
if (Test-Path $Binary)
{
	$FileVersionInfo = [System.Diagnostics.FileVersionInfo]::GetVersionInfo($Binary)
	
	$InformationCollected | add-member -membertype noteproperty -name "HP WBEM provider version detected" -value $FileVersionInfo
	
	if ($FileVersionInfo.FileMajorPart -lt 10)
	{
		#hp binary version with known issue detected
		$HPWbemUpdateRequired = $true
	}
	elseif(($FileVersionInfo.FileMajorPart -le 10) -and ($FileVersionInfo.FileMinorPart -le 10))
	{
		#hp binary version with known issue detected
		$HPWbemUpdateRequired = $true
	}
	...
	{
		#hp binary version with known issue not detected
		$HPWbemUpdateRequired = $false
	}
	
	
	#detect registry damage
	$autorecoverMofsCount = ((Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Wbem\CIMOM\")."Autorecover MOFs").count 
	if ($autorecoverMofsCount -le 400)
	{
		#"Autorecover MOFs registry key has been damaged"
		$damagedAutorecoverMofsRegistryKey = $true
		$InformationCollected | add-member -membertype noteproperty -name "Count of Autorecover Mofs entries" -value $autorecoverMofsCount
	}
}

#identify root cause detected
if(($damagedAutorecoverMofsRegistryKey -eq $true) -and ($HPWbemUpdateRequired -eq $true))
{
	#root cause detected, error
	$rootCauseDetected = $true
}
elseif ($HPWbemUpdateRequired -eq $true)
{
	$rootCauseDetected = $false
}
else
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCArbxmfDkyJKLej
# ZiSbez1RpLKDL7Ada8ZO3zkIt0bz86CCDYEwggX/MIID56ADAgECAhMzAAAB32vw
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQg0eK57JuJ
# N1dRYc68dkFVzS9PJ6cyDRBa571fJnloofQwOAYKKwYBBAGCNwIBDDEqMCigCIAG
# AFQAUwBToRyAGmh0dHBzOi8vd3d3Lm1pY3Jvc29mdC5jb20gMA0GCSqGSIb3DQEB
# AQUABIIBAAsHnusVP81uxSUCVLiXwD0MQEU0NF3z63sB56q5b6ZrU3Pc0eJl7ZRY
# 6a50TSkA0zCqif4AANPNMHbSLZOoOSr56jRBugU41ay0bBP8s/au0NU+nlENzAjm
# QQV7uXVkan/WXZGfCAG8B08Lb9ImPLN8mNfkX8uWW7Abhf1CfPc9roXSMFN++tAw
# l6SY8EJwtX10jGgfcvKVrVylXkGiP8p6Og/PKg9e/AYeasoBR3DkalY0qrxZQL7L
# vlgVeU/nIPXRO8pMGhcIcP879cQVIYr/lA5rjWnRFlobZALi2qvSlLKnJ5FQsfyr
# f7JqpLxzD+FR/oxlYUei8+3X03JC14KhghLxMIIS7QYKKwYBBAGCNwMDATGCEt0w
...
# ZQMEAgEFAAQgYbi0LUX2c1CkchR/WD5VE1pF4VLEhw+iJcfQmJR/+ncCBmGBshkU
# ixgTMjAyMTExMTExNjUzMzIuNzg2WjAEgAIB9KCB1KSB0TCBzjELMAkGA1UEBhMC
...
# GgYJKoZIhvcNAQkDMQ0GCyqGSIb3DQEJEAEEMC8GCSqGSIb3DQEJBDEiBCBNMWoP
# gn//+Ga9HlwwG0pWAUgWRiKZ6GiNw+8RwaAKEDCB+gYLKoZIhvcNAQkQAi8xgeow
...
# hvcNAQELBQAEggEAEUFJlB3Pfm0lvDxg76yyJG9UZfljLton3cZkTHEsHsX/6Qbi
# hmdPHeRP7oCa7F/Vg8VdjUlF+efzhSICzgsE1ZfSAS63DhhZqqBfoT2OlG0FuGli
# S0tkrt4Pl3t/QjtJqogDbrFXlOaTiCdKYNT5lH5SBfiTs62G05hcystKbmUasJUa
# qRKuStpsGG2IVGwbDnCx9kVPXBEfq4FNv2qKgk5gnGpmazIHzsC2M4Kv1OHeeJMu
# dxc/yhnnq5aI6rfXOKRVJ6lIim96l5Z0txpAcAQZpNU5TgDz0TBfRpWl2+MdDbbw
# yvT/e4qLwceFoCxVSVlfJl6O1x8FzOTA2n3ZLQ==
