...
# Rule number:  7510
# Rule URL:  http://sharepoint/sites/rules/Rule Submissions/Forms/DispForm.aspx?ID=7510
#************************************************

Import-LocalizedData -BindingVariable ScriptStrings
Display-DefaultActivity -Rule -RuleNumber 7510

$RuleApplicable = $false
$RootCauseDetected = $false
$RootCauseName = "RC_ModernAppsFailureForRegistryPermissionCheck"
...
Function CheckAllAppPackagesPermissionsonRegistry()
{
    $RegsToBeCheck = @("Registry::HKEY_CLASSES_ROOT","REGISTRY::HKEY_USERS","HKLM:\Drivers","HKLM:\HARDWARE","HKLM:\SOFTWARE","HKLM:\SYSTEM","HKLM:\SAM")	
	$RegistryRightsRequired = @([System.Security.AccessControl.RegistryRights]::QueryValues,[System.Security.AccessControl.RegistryRights]::EnumerateSubKeys,[System.Security.AccessControl.RegistryRights]::Notify,[System.Security.AccessControl.RegistryRights]::ReadPermissions,[System.Security.AccessControl.RegistryRights]::WriteKey,[System.Security.AccessControl.RegistryRights]::ReadKey,[System.Security.AccessControl.RegistryRights]::FullControl)			
	$RegMissingPermissionExists = $false
	Foreach($reg in $RegsToBeCheck)
	{
	    if(Test-Path -Path $reg)
		{		    
			$AllApplicationPackagesAccess = (get-acl -path $reg).access | Where-Object -FilterScript {$_.IdentityReference.value -eq "APPLICATION PACKAGE AUTHORITY\ALL APPLICATION PACKAGES"}			
			$RegQualified = $false
			if($null -eq $AllApplicationPackagesAccess)
			{
				$RegistryRightsProvided = $null
			}
			else{
				foreach($Access in $AllApplicationPackagesAccess)
				{
					if($RegQualified -eq $false)
					{
						$RegistryRightsProvided = [Enum]::GetValues([System.Security.AccessControl.RegistryRights]) |Where-Object {$_ -band $Access.RegistryRights}
						$RegistryRightsRequired | ForEach-Object {
	   			 			if($RegistryRightsProvided -notcontains $_)
	   						{
	       						break
	   						}			   
						}
		 	...
		 	if($RegQualified -eq $false) 
			{							
				$RegMissingPermissionExists = $true
				$InformationCollected = New-Object PSObject
				Add-Member -InputObject $InformationCollected -MemberType NoteProperty -Name "Account" -Value "ALL APPLICATION PACKAGES"
				Add-Member -InputObject $InformationCollected -MemberType NoteProperty -Name "Hive with missing permissions" -Value (Convert-Path $reg)												
				if($null -ne $RegistryRightsProvided)
				{
					$CurrentPermissions = [string]::Join(", ",$RegistryRightsProvided)   
				}
				else
				{
					$CurrentPermissions = "No Permission"
				}
				Add-Member -InputObject $InformationCollected -MemberType NoteProperty -Name "Current Permissions" -Value $CurrentPermissions
				Add-Member -InputObject $InformationCollected -MemberType NoteProperty -Name "Expected Permissions" -Value [string]::Join(", ",$RegistryRightsRequired)
				Add-GenericMessage -Id $RootCauseName -InformationCollected $InformationCollected 
			}
		}
	}
	return $RegMissingPermissionExists
}
...
if (AppliesToSystem)
{
	...
	if (CheckAllAppPackagesPermissionsonRegistry)
	{
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCkzmbzfx8+ztMo
# ur1OP5ew8CqIY7p60YgHFTNUk+KfiaCCDXYwggX0MIID3KADAgECAhMzAAACy7d1
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIB4gyOI3caU5UH9GFtQDdNdi
# ZzvMcMfs7Zmw/zM8g/zzMEQGCisGAQQBgjcCAQwxNjA0oBSAEgBNAGkAYwByAG8A
# cwBvAGYAdKEcgBpodHRwczovL3d3dy5taWNyb3NvZnQuY29tIDANBgkqhkiG9w0B
# AQEFAASCAQ