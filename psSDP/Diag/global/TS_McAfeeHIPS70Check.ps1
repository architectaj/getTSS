#************************************************
# TS_McAfeeHIPS70Check.ps1
# Version 1.0.1
# Date: 05/06/2013
# Author: v-alyao
# Description:  [Idea ID 7521] [Windows] McAfee HIPS 7.0 adds numerous extraneous network adapter interfaces to registry
# Rule number:  7521
# Rule URL:  http://sharepoint/sites/rules/Rule Submissions/Forms/DispForm.aspx?ID=7521
#************************************************

Import-LocalizedData -BindingVariable ScriptStrings
Display-DefaultActivity -Rule -RuleNumber 7521

$RuleApplicable = $false
$RootCauseDetected = $false
$RootCauseName = "RC_McAfeeHIPS70Check"
...
Function AppliesToSystem
{
	...
	if((($OSVersion.Major -eq 6) -and  ($OSVersion.Minor -eq 0)) -or   #Windows Server 2008, Windows Vista
	(($OSVersion.Major -eq 6) -and  ($OSVersion.Minor -eq 1)))   #Windows 7,Windows Server 2008 R2
	{
		if($OSArchitecture -eq "X86")
		{
			$HipPath = "HKLM:\SOFTWARE\McAfee\HIP"
		}
		elseif($OSArchitecture -eq "AMD64")
		{
			$HipPath = "HKLM:\SOFTWARE\Wow6432Node\McAfee\HIP"
		}
		if(Test-Path $HipPath)
		{
			$Hip = Get-ItemProperty $HipPath
			if(($null -ne $Hip) -and ($null -ne $Hip.Version))
			{
				("[Info] McAfee HIPS detected with " + $Hip.Version) | WriteTo-StdOut -ShortFormat
				if($Hip.Version.StartsWith('7'))
				{
					$InformationCollected | add-member -membertype noteproperty -name "McAfee HIPS Version" -value ($Hip.Version)
					return $true
				}
				else
				{
					"[Info] McAfee HIPS 7.0 not detected" | WriteTo-StdOut -ShortFormat
					return $false
				}
			}
			else
			{
				"[Info] McAfee HIPS' version not detected" | WriteTo-StdOut -ShortFormat
				return $false
			}
		}
		else
		{
			"[Info] McAfee HIPS not detected" | WriteTo-StdOut -ShortFormat
			return $false
		}
	}
	else
	{
		"[Info] Only applies to Windows Server 2008/Windows Vista/Windows 7/Windows Server 2008 R2" | WriteTo-StdOut -ShortFormat
		return $false
	}
}

Function CheckMcAfeeHIPS70()
{
	$NDISPath = "HKLM:\SYSTEM\CurrentControlSet\services\NDIS\IfTypes\1"
	if(Test-Path $NDISPath)
	{
		$NDIS = Get-ItemProperty $NDISPath
		if(($null -ne $NDIS) -and ($null -ne $NDIS.IfUsedNetLuidIndices))
		{
			$numberOfFFBytes = 0
			foreach($item in $NDIS.IfUsedNetLuidIndices)
			{
				if($item -eq 255)
				{
					$numberOfFFBytes++
			...
			if($numberOfFFBytes -ge 50)
			{
				#Add information to InformationCollected
				$InformationCollected | add-member -membertype noteproperty -name "Number of NDIS FF bytes" -value $numberOfFFBytes
				return $true
			}
			else
			{
				"[Info] Number of NDIS FF bytes: $numberOfFFBytes" | WriteTo-StdOut -ShortFormat
				return $false
			}
		}
	}
...
{
	$RuleApplicable = $true
	$RootCauseDetected = CheckMcAfeeHIPS70	
}	
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC