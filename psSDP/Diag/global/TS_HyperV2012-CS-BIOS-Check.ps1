#************************************************
# TS_HyperV2012-CS-BIOS-Check.ps1
# Version 1.0.1
# Date: 05/08/2013
# Author: v-alyao
# Description:  [Idea ID 5752] [Windows] BIOS update may be required for some computers before starting Hyper-V on 2012
# Rule number:  5752
# Rule URL:  http://sharepoint/sites/rules/Rule Submissions/Forms/DispForm.aspx?ID=5752
#************************************************

Import-LocalizedData -BindingVariable ScriptStrings
Display-DefaultActivity -Rule -RuleNumber 5752

$RuleApplicable = $false
$RootCauseDetected = $false
$RootCauseName = "RC_HyperV2012-CS-BIOS-Check"
...
Function AppliesToSystem
{
	...
	if(($OSVersion.Major -eq 6) -and  ($OSVersion.Minor -eq 2))#Windows 8/Windows Server 2012
	{
		$HyperVServicePath = "HKLM:\SYSTEM\CurrentControlSet\services\vmms"
		if(Test-Path $HyperVServicePath)
		{
			"[Info] Hyper-V is installed" | WriteTo-StdOut -ShortFormat
			$requiredModelList = @('PowerEdge R510','PowerEdge R710','PowerEdge R910')
			$computerInfo = Get-CimInstance -Class Win32_ComputerSystem
			if($requiredModelList -contains $computerInfo.Model)
			{
				#Add information to InformationCollected
				$InformationCollected | add-member -membertype noteproperty -name "System Model" -value ($computerInfo.Model)
				return $true
			}
			else
			{
				"[Info] Only Applies to R510, R710, R910 Model" | WriteTo-StdOut -ShortFormat
				return $false
			}
		}
		else
		{
			"[Info] Hyper-V is not installed" | WriteTo-StdOut -ShortFormat
			return $false
		}
	}
	else
	{
		"[Info] Only Applies to Windows 8/Windows Server 2012" | WriteTo-StdOut -ShortFormat
		return $false
	}
}

Function CheckFirmwareReleaseDate()
{
	$biosInfo = Get-CimInstance -Class win32_Bios
	if(($null -ne $biosInfo.ReleaseDate) -and ($biosInfo.ReleaseDate.Length -ge 8))
	{
		if($biosInfo.ReleaseDate.Substring(0,8) -le '20120901')
		{
			$InformationCollected | add-member -membertype noteproperty -name "Required BIOS Release Date (or newer)" -value '20120901'
			$InformationCollected | add-member -membertype noteproperty -name "Current BIOS Release Date" -value ($biosInfo.ReleaseDate)
			return $true
		}
		else
		{
			"[Info] Current BIOS Release Date: " + $biosInfo.ReleaseDate + ". It is newer than Sept 2012, rule will not fire." | WriteTo-StdOut -ShortFormat
			return $false
		}
	}
	else
	{
		"[Error] Cannot get the ReleaseDate or it's invalid." | WriteTo-StdOut -ShortFormat
		return $false
	}
}
...
{
	$RuleApplicable = $true
	$RootCauseDetected = CheckFirmwareReleaseDate	
}	
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAMdot7rjwZDqW2
# rAzn3fBDTnNReojbnyPvoy/Pk0pEEqCCDXYwggX0MIID3KADAgECAhMzAAACy7d1
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIFkJ3pXKwZi5nQHoCvfqlMBp
# L8AxKemmdmRiiDRUNZ5cMEQGCisGAQQBgjcCAQwxNjA0oBSAEgBNAGkAYwByAG8A
# cwBvAGYAdKEcgBpodHRwczovL3d3dy5taWNyb3NvZnQuY29tIDANBgkqhkiG9w0B
# AQEFAASCAQ