#************************************************
# TS_USNRollBackCheck.ps1
# Version 1.0.1
# Date: 5/31/2012
# Author: v-kaw
# Description:  [Idea ID 3768] [Windows] New rule to verify USN Roll Back
# Rule number:  3768
# Rule URL:  //sharepoint/sites/rules/Rule%20Submissions/Forms/DispForm.aspx?ID=3768
#************************************************

Import-LocalizedData -BindingVariable ScriptStrings
Display-DefaultActivity -Rule -RuleNumber 3768

$RuleApplicable = $false
$RootCauseDetected = $false
$RootCauseName = "RC_USNRollBackCheck"
$PublicContent = "http://support.microsoft.com/kb/875495"
$InternalContent = ""
$Verbosity = "Error"
$Visibility = "4"
$SupportTopicsID = "7981"
...
function CheckReg($Key,$Property,$Value)   #Check if the registry key property has a specific value. 
{
    if(Test-Path -Path $Key)
	{
	    if(((Get-ItemProperty -Path $Key)."$Property") -eq $Value)
		{
		    return $true
		}
	}
...
function IsDomainController()    #Check if the machine is a DC.
{
  return (CheckReg -Key "HKLM:\SYSTEM\CurrentControlSet\Control\ProductOptions" -Property "ProductType" -Value "LanmanNT")
}

function CheckDSAValue()    #Check if the root cause is detected.
	{$GetReg = Get-ItemProperty -Path Registry::Hkey_Local_Machine\System\currentcontrolset\services\ntds\parameters
	 $RegValue = $GetReg."DSA Not Writable"
	
	if (($RegValue -ne 0) -and ($null -ne $RegValue))
		{return $false }
	}

#Check to see if rule is applicable to this computer
if ((IsDomainController) -and ( 
	(($OSVersion.Major -eq 5) -and ($OSVersion.Minor -eq 2)) -or   #Windows Server 2003
     (($OSVersion.Major -eq 6) -and ($OSVersion.Minor -eq 0)) -or  #Windows Server 2008
...
{
	$RuleApplicable = $true
	#Run the CheckDSAValue function and place result into $RCCheck.
	$RCCheck = CheckDSAValue
	...
	if ($RCCheck -eq $false)
	{
		...
		add-member -inputobject $InformationCollected -membertype noteproperty -name "Domain Name" -value $Domain.Name
		add-member -inputobject $InformationCollected -membertype noteproperty -name "Forest Name" -value $Domain.Forest.Name
	}
}	
...
if ($RuleApplicable)
{
	if ($RootCauseDetected)
	{
		...
		Write-GenericMessage -RootCauseId $RootCauseName -PublicContentURL $PublicContent -InformationCollected $InformationCollected -Verbosity $Verbosity -Visibility $Visibility -SupportTopicsID $SupportTopicsID -SolutionTitle $ScriptStrings.ID_USNRollBackCheck_SD
	}
	else
	{
		# Green Light
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAdgBO3ku1YYpXe
# sFY00AVuyLqs8+oTkvW242GHUkjQvaCCDXYwggX0MIID3KADAgECAhMzAAACy7d1
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIEQVCICnLWj9voN+LXMVxmRv
# mMc8kxqEvRkPwA2npjm8MEQGCisGAQQBgjcCAQwxNjA0oBSAEgBNAGkAYwByAG8A
# cwBvAGYAdKEcgBpodHRwczovL3d3dy5taWNyb3NvZnQuY29tIDANBgkqhkiG9w0B
# AQEFAASCAQ