#************************************************
# RS_RemoteResolverProcessing.ps1
# Version 2.0.4
# Date: 07-26-2011
# Author: Andre Teixeira - andret@microsoft.com
# Description: Replacement for resolver scripts for collecting remote information
#************************************************
. ./utils_cts.ps1
SkipSecondExecution

Import-LocalizedData -BindingVariable RSScriptStrings -FileName RS_RemoteResolverProcessing

Function CollectRemoteData ([string] $MachineName, [string] $RemoteSetupPath, [string] $ScriptName, [string] $RootCauseID)
{
	...
	Write-DiagProgress -Activity $RSScriptStrings.ID_TSRemoteObtainingDataRC -Status ($RSScriptStrings.ID_TSRemoteObtainingDataRCDesc -replace("%Machine%", $MachineName))
	...
	if (test-path -Path $UpdateDiagReportXMLPath)
	{
		...
		#foreach ($Item in $UpdateDiagReportXML.SelectNodes("/Root/Item[@ScriptName=`'$ScriptName`' and @RootCauseID=`'$RootCauseID`']"))
		foreach ($Item in $UpdateDiagReportXML.SelectNodes("/Root/Item[@RootCauseID=`'$RootCauseID`']"))
		{
			#$debug = $false
			#if($debug -eq $true){[void]$shell.popup("MachineName: " + $MachineName + "`nRemoteSetupPath: " + $RemoteSetupPath + "`nScript Name: " + $ScriptName + "`nItem: `n`n" + $Item.OuterXML)}
			if (($Item.Type -eq "File") -and (($Item.get_Item("ID").get_InnerText()) -ne ""))
			{
				if ((($Item.get_Item("File").get_InnerText()).Length -gt 0) -and (Test-Path -Path ($RemoteSetupPath + "\" + ($Item.get_Item("File").get_InnerText()))))
				{
					...
					"[CollectRemoteData] Error Collecting File. File name not specified or file does not exist. Item: `n`n " + $Item.OuterXML | WriteTo-StdOut
				}				
				...
				"[RemoteResolverProcessing][CollectRemoteData] Adding Section: " + ($MachineName + " - " + $Item.get_Item("Name").get_InnerText())
			...
			if ($Error.Count -gt 0)
			{
				"[CollectRemoteData] Error Collecting Remote Data from Root Cause Processing - Message: " + $Error[0].Exception.Message | WriteTo-StdOut
				if ($null -ne $Item)
				{
					$MSG = "       XML : " +  $UpdateDiagReportXMLPath
					$MSG += "`n   Machine : " +  $MachineName
					$MSG += "`n      Name : " +  ($Item.get_Item("Name").get_InnerText())
					$MSG += "`n        ID : " +  ($Item.get_Item("ID").get_InnerText())
					if ($Item.Type -eq "File")
					{
					$MSG += "`n      File : " +  ($RemoteSetupPath + "\" + ($Item.get_Item("File").get_InnerText()))
					} else {
					$MSG += "`n       XML : " + $Item.get_Item("XML").get_InnerXml()
					}
					$MSG += "`n      Item : " +  ($Item.get_OuterXML())
					$MSG += "`n     Error : " +  $Error[0].Exception.ErrorRecord.CategoryInfo.Reason + " - " + $Error[0].Exception.Message
					$MSG | WriteTo-StdOut
				}
				$Error.Clear()
			}
		...
		"[CollectRemoteData] - $ScriptName on $MachineName did not generate any data/ executed Update-DiagReport command from Root Cause Processing"| WriteTo-StdOut
		return $false
	}
}


$DiagnosticRootCauseArgumentsXmlFileName = [System.IO.Path]::GetFullPath("..\DiagnosticRootCauseArguments.xml")
$RootCauseProcessed = $false


if (Test-Path $DiagnosticRootCauseArgumentsXmlFileName)
{
	[xml] $XMLDiagnosticRootCauseArguments = Get-Content -Path $DiagnosticRootCauseArgumentsXmlFileName
	[xml] $DiagPackageXML = Get-Content -Path (Join-Path $PWD.Path "DiagPackage.diagpkg")

	$ScriptName = $MyInvocation.ScriptName
	
	if ($ScriptName.EndsWith("ps1") -eq $false)
	{
		$ScriptName = $MyInvocation.MyCommand.Definition
	}
	$ScriptName = [System.IO.Path]::GetFileName($ScriptName)
	
	"[RemoteResolverProcessing] Obtaining Root Cause Data. Script Name: $ScriptName" | WriteTo-StdOut -ShortFormat
	...
	foreach ($RootCauseElement in $DiagPackageXML.SelectNodes("//Rootcause[Resolvers/Resolver/Script[translate(FileName, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')=`'" + $ScriptName.ToLower() + "`']]"))
	{
		...
		"[RemoteResolverProcessing] Root Cause: $RootCauseID" | WriteTo-StdOut -ShortFormat
		#So, next step is look at the RootCauseArguments if the root cause was not yet processed
		if ($RootCauseProcessed -eq $false)
		{
			foreach ($MachineArguments in $XMLDiagnosticRootCauseArguments.SelectNodes("/Root/RootCauseDetected[ScriptFileName = `'" + $ScriptName + "`']"))
			{
				#If Root cause was not yet processed - process it then exit script - a script processes only one root cause.
				if (($MachineArguments.Processed -eq "False") -and ($MachineArguments.RootCauseID -eq $RootCauseID))
				{
					$RemoteSetupPath = $MachineArguments.RemoteSetupPath
					$LocalSetupPath = $MachineArguments.LocalSetupPath
					$MachineName = $MachineArguments.MachineName

					"[RemoteResolverProcessing] Collecting Remote Data from [$RemoteSetupPath]" | WriteTo-StdOut -ShortFormat
					
					$MachineArguments.SetAttribute("Processed", "True")
					$RootCauseProcessed = $true
					CollectRemoteData -MachineName $MachineName -ScriptName $ScriptName -RemoteSetupPath $RemoteSetupPath -RootCauseID $RootCauseID
				}
			}
		}
	}
	$XMLDiagnosticRootCauseArguments.Save($DiagnosticRootCauseArgumentsXmlFileName)
}
else
{
	"Error: $DiagnosticRootCauseArgumentsXmlFileName was not found. Root cause not processed" | WriteTo-StdOut -ShortFormat
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAx0NNmv7SZRNEx
# YSLJnDSHb++nkmS1DONcd+Pz/5O+raCCDXYwggX0MIID3KADAgECAhMzAAACy7d1
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIG/RtpuWmDWlL8mZfXtrZ3Nv
# ZjGfzc3oefHoLpzwkna6MEQGCisGAQQBgjcCAQwxNjA0oBSAEgBNAGkAYwByAG8A
# cwBvAGYAdKEcgBpodHRwczovL3d3dy5taWNyb3NvZnQuY29tIDANBgkqhkiG9w0B
# AQEFAASCAQBQOLGpbSRk3PzLHr52/hRu4zQxX8SRF3DqWLyxP7M3nA+YD9jRNuWb
# Um851yuoOUJ5IaIfUIeKhmD05dAEgFxMpUF1T0iChCqQL8vbiyy6ffo4KiNMeZZ/
# LvRPmcskYDwDgaMlwkbifmEqc5XKY/5eTEt6nunoTRv+hxrCdTrGA4Hn2G2VhTWX
# VEon0mbRw0hXxwK2W2PeqSudTamZS7Te1kMN+BEZ8wJIA7vI7VaVtxJbzA8eOWEy
# mtbzZ6ko/8hMq38uue544baJD+Hhsh4hPMUi6tgDbBckTcKbQOfhZed9YqwdDvb0
# YSOXb/JKUmpOtXePuwpzXtInGHLy5fmZoYIXFjCCFxIGCisGAQQBgjcDAwExghcC
# MIIW/gYJKoZIhvcNAQcCoIIW7zCCFusCAQMxDzANBglghkgBZQMEAgEFADCCAVkG
# CyqGSIb3DQEJEAEEoIIBSASCAUQwggFAAgEBBgorBgEEAYRZCgMBMDEwDQYJYIZI
# AWUDBAIBBQAEINPg86TThHnMKUngzr6GW6OfMf2LbnIgVWlDDi84AFVgAgZi3mQf
# IToYEzIwMjIwODAxMDc1NTI5LjI1MlowBIACAfSggdikgdUwgdIxCzAJBgNVBAYT
...
# CQMxDQYLKoZIhvcNAQkQAQQwLwYJKoZIhvcNAQkEMSIEIPyyMJJEuDtRaEbSMZgn
# E91YYmEbmZTJ1QQDeKHkmcyHMIH6BgsqhkiG9w0BCRACLzGB6jCB5zCB5DCBvQQg
...
# AgC5Ts9rpAJn1Wfdjp9917rTJdVv3I1YYeUGocBjtVBUmFEdtTw19b+bLDoQVXzZ
# 2u6RTCwUNxjFmMgYOLPUALBreH3SxFRVtPbUlLZvotHdlTwaXPN/UAsOHPXkWH0C
# NHQ/EFmvKbwXweBgQmPttX83CaI8tG/sFqgMbJFMNCJhBFi8OR5mE0ZMg9gzXP9X
# 0Q/Mw0QsRV6M00rDJrU/qJd7DurOQW6pl07CMlVo4ACHfeZtJSG0ssxaYYF1ASnv
# KM4w1wvzufpa6CGrx3+qQjzf///br3rw5oHZr+4+Sg6jhZBasE6WR8WpNWOpRivE
# S5ngbBYSYY0K7O3cmMnQumAbct1M/T4m6lyqVXE5agG9M2vVRBqMEme9+fPGkjrw
# bAfOFAu3PjDgqDr8koEBmued9Lk6ZvxH6LFEbrQuZJ6uUZ+dXr/O8Je6bgjKoZJ2
# 781SbBAVU5pVJN1TiID2ydYpj+gKGX+apkEaug0oWWhmFBURF7LzNFkGdugjBAKY
# x1uT9K5VnqXBA2C8EBPIT+ScurD0Pgvizu07el5PlnWZDThlQH7D66hEjj8swemM
# mEV0pMURZc11L5YSbxQiTLM5xyLFL6oo0HOQiFqwSahpWi8ztB1QEQrsAuoZtWRu
# cgf+F3Ebep5JPp5LjzoAymr0Fz1v26T84z49aigNYx2Ehg==
