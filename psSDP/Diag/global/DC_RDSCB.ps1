...
$Deployment = Get-CimInstance -Namespace ROOT\cimv2\rdms -Class Win32_RDMSDeploymentSettings -List -ComputerName $TargetHost  -Authentication PacketPrivacy -Impersonation Impersonate
...
foreach( $Cluster in $RDSCBCluster)
{
	...
	$RDSCBClusterEnum = Get-CimInstance -Query $Query -Namespace root\cimv2 -ComputerName $TargetHost -Authentication PacketPrivacy -Impersonation Impersonate
	if ($null -ne $RDSCBClusterEnum) 
	{
		[array]$CBLog +=""
		...
		ForEach($RDSCBClusterNode in $RDSCBClusterEnum)
		{
			...
			$RDSCBSessionEnum = Get-CimInstance -Query $Query -Namespace root\cimv2 -ComputerName $TargetHost -Authentication PacketPrivacy -Impersonation Impersonate
			if ($null -ne $RDSCBSessionEnum) 
			{
				[array]$CBLog +=""
...
$RDSCBVMPDFarms = Get-CimInstance -Query $Query -Namespace root\cimv2 -ComputerName $TargetHost -Authentication PacketPrivacy -Impersonation Impersonate
if ($null -ne $RDSCBVMPDFarms)
{
	[array]$CBLog +=""
	...
	ForEach($RDSCBVMPDFarm in $RDSCBVMPDFarms)
	{
		...
		$RDSCBVMPDTarget = Get-CimInstance -Query $Query -Namespace root\cimv2 -ComputerName $TargetHost -Authentication PacketPrivacy -Impersonation Impersonate
		if ( $null -ne $RDSCBVMPDTarget)
		{
			[array]$CBLog +=""
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBxso+DmQo3dgXM
# b88Y90mir7fFLvOvn4MRHYVsr8A2R6CCDXYwggX0MIID3KADAgECAhMzAAACy7d1
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIAR7Thz0eV9Qit5yJoovp2aE
# hL6DPdTmimemL8QrwYlRMEQGCisGAQQBgjcCAQwxNjA0oBSAEgBNAGkAYwByAG8A
# cwBvAGYAdKEcgBpodHRwczovL3d3dy5taWNyb3NvZnQuY29tIDANBgkqhkiG9w0B
# AQEFAASCAQAllIaJ1HBlFNBwsTFI6figoTtg1606skkHoKz/7o6MW+EZ6rgDoEly
# Bzb/nn7FAW7nIWJAo9d7Co7ijyerEjeZf11Kd/YmBij8R4CXFV73jpY2kuU2kXkC
# aMUGWCa/15OnZSc0nuJF/ToasMTUaRP3gdgf21kAwmZl6RSEn7hJKi69RCsDWnHg
# duA9p7BJz9mifwi+FlQCXJKd2i1OLyt1WardfLxSqVt1ccV+Wih14+PKvXJ6zQBg
# ig/tEA20PyDxrfqkkgP31Sx+GsXCCtLuGosPnZ83QV5CHc/6MCE9V4L6hdnAwleI
# GtS7yz9rUpxsPwnXvhF19O/JSGRcamRDoYIXFjCCFxIGCisGAQQBgjcDAwExghcC
# MIIW/gYJKoZIhvcNAQcCoIIW7zCCFusCAQMxDzANBglghkgBZQMEAgEFADCCAVkG
# CyqGSIb3DQEJEAEEoIIBSASCAUQwggFAAgEBBgorBgEEAYRZCgMBMDEwDQYJYIZI
# AWUDBAIBBQAEIPFUngZ3rFKx6q8/9S/upiQYfoejwkOVoSt3Q+r87/qrAgZi3mrX
# 8eMYEzIwMjIwODAxMDc1MDEwLjE1NFowBIACAfSggdikgdUwgdIxCzAJBgNVBAYT
...
# CQMxDQYLKoZIhvcNAQkQAQQwLwYJKoZIhvcNAQkEMSIEIPyB33nCzIdQfLY+c1C2
# BE8QYR+uQ+jsq79VDTC0gOtDMIH6BgsqhkiG9w0BCRACLzGB6jCB5zCB5DCBvQQg
...
# AgBXbADOwu70/jpcgAZtbOYxi81a2fM0Q4G5KaMHtXyQ3suNAOgyuVrTS+MbdCtz
# FMDVr3/hBVv2reJNRQGKWiuse7UofUyFx/0ZZlwT7xeHL9oBwL1asPe9GE4Nb06b
# UYzJrTqrO1GWqreQkxWI0SmwrRQyTYDC1vb7zx41NlwjLv/RGtVg1EVDDlC9mTB8
# MdRzYmk5t/HnM+mfNmA1tNXjnYrTP84RiC8qkNeTH43I83MBTxwOqMynfQJEq2NM
# lTqcdMRHr/5Wpk/zhdXh2CTIImKGga5Fd9i2wDOvX1RKhxuZOZsqN8nTY3ZsxCXP
# KiUJIiMIlNZuAUuXVYr+Yz6rV2MT3leGnDQ4Sv02xutUHlCTMh6lGWeeDeDJgDMP
# hOlU24Brmg2xyP7M8xzJ+JxKmDM0P94xxOf2Le4jD5DQF5T+SGlA13tL581MLGa4
# fmlIo6nA87Y9N2aBsZqNIiM5PEF1FDoBkXAJRlKkYWSJXCeGrKr1KqnOz6zJWVC1
# kMr52zLZALS4U8pNJ6bhIJ2+Mgk1ohNe7otY9VeF9ble7YDxwIVOzwgvdNX96VLm
# AqUCrK+3srT7g2rDZbO61MvADRA8rysiCFrvD9HKgfRs7Ge4Tk4pfa29wMyB8HSY
# 1Qs21xIKOhZAlX93KSfoJmqzHw1HrmqhJmApYrV540xb0w==
